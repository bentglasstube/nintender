...
#define STR_MANUFACTURER L"ABCF"
#define STR_PRODUCT      L"Nintender"
#define VENDOR_ID        0xabcf
...
uint8_t gamepad_axis_x = 128;
uint8_t gamepad_axis_y = 128;
...
void usb_gamepad_from_nes(uint16_t data) {
  ...
  gamepad_buttons = (data & 0xc0) + ((data & 0x30) >> 4);

       if (data & 0x08) gamepad_axis_y = 0x00;
  else if (data & 0x04) gamepad_axis_y = 0xff;
  else                  gamepad_axis_y = 0x80;

       if (data & 0x02) gamepad_axis_x = 0x00;
  else if (data & 0x01) gamepad_axis_x = 0xff;
  else                  gamepad_axis_x = 0x80;
}

int8_t usb_gamepad_send() {
  ...
  UEDATX = gamepad_axis_x;
  UEDATX = gamepad_axis_y;
